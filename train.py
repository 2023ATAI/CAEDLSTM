...
from model import LSTMModel,CNN,ConvLSTMModel,LSTMEncoderDecoder,AttentionLSTM,LSTMAttentionEncoderDecoder
...
#	filter Antatctica
    ...
    if cfg['modelname'] in ['LSTM','LSTMEncoderDecoder','AttentionLSTM','LSTMAttentionEncoderDecoder']:
        if valid_split:
	...
	#Selection model
        ...
        elif cfg['modelname'] in ['LSTMEncoderDecoder']:
            model = LSTMEncoderDecoder(cfg).to(device)
        elif cfg['modelname'] in ['AttentionLSTM']:
            model = AttentionLSTM(cfg).to(device)
        elif cfg['modelname'] in ['LSTMAttentionEncoderDecoder']:
            model = LSTMAttentionEncoderDecoder(cfg).to(device)
 ...
 #  train way for LSTMEncoderDecoder model
                        ...
                        pred = model(x_batch)
                        pred = torch.squeeze(pred,1)
 #  train way for AttentionLSTM model
                    elif cfg["modelname"] in \
                            ['AttentionLSTM']:
                        # generate batch data for Recurrent Neural Network
                        x_batch, y_batch, aux_batch, _, _ = \
                        ...
                        pred = model(x_batch)
                        pred = torch.squeeze(pred,1)
 #  train way for AEDLSTM model
                    elif cfg["modelname"] in \
                            ['LSTMAttentionEncoderDecoder']:
                        # generate batch data for Recurrent Neural Network
                        x_batch, y_batch, aux_batch, _, _ = \
                            load_train_data_for_rnn(cfg, x, y, static, scaler_y)
                        x_batch = torch.from_numpy(x_batch).to(device)
...
#  validate way for LSTMEncoderDecoder model
                        elif cfg["modelname"] in ['LSTMEncoderDecoder']:
                            gt_list = [i for i in range(0, x_valid.shape[0] - cfg['seq_len'], cfg["stride"])]
                            n = (x_valid.shape[0] - cfg["seq_len"]) // cfg["stride"]
                            for i in range(0, n):
                                # mask
                                x_valid_batch, y_valid_batch, aux_valid_batch, _, _ = \
                                    load_test_data_for_rnn(cfg, x_valid, y_valid, static_valid, scaler_y,
                                                           cfg["stride"], i, n)
                                x_valid_batch = torch.Tensor(x_valid_batch).to(device)
                                y_valid_batch = torch.Tensor(y_valid_batch).to(device)
                                aux_valid_batch = torch.Tensor(aux_valid_batch).to(device)
                                ...
                                mse_valid_loss = NaNMSELoss.fit(cfg, pred_valid.squeeze(1), y_valid_batch,
                                                                lossmse)
                                MSE_valid_loss += mse_valid_loss.item()
#  validate way for 'AttentionLSTM' model
                        elif cfg["modelname"] in ['AttentionLSTM']:
                            gt_list = [i for i in range(0, x_valid.shape[0] - cfg['seq_len'], cfg["stride"])]
                            n = (x_valid.shape[0] - cfg["seq_len"]) // cfg["stride"]
                            for i in range(0, n):
                                ...
                                mse_valid_loss = NaNMSELoss.fit(cfg, pred_valid.squeeze(1), y_valid_batch,
                                                                lossmse)
                                MSE_valid_loss += mse_valid_loss.item()
#  validate way for ' AEDLSTM' model
                        elif cfg["modelname"] in ['LSTMAttentionEncoderDecoder']:
                            gt_list = [i for i in range(0, x_valid.shape[0] - cfg['seq_len'], cfg["stride"])]
                            n = (x_valid.shape[0] - cfg["seq_len"]) // cfg["stride"]
                            for i in range(0, n):
