        ...
        for i in range(x.shape[1]):
            for j in range(x.shape[2]):
              ...
              #  print('pred reverse is',pred)
                ...
                count = count+1
# ------------------------------------------------------------------------------------------------------------------------------
    # for each grid by LSTMEncoderDecoder model
    if cfg["modelname"] in ['LSTMEncoderDecoder']:
        count = 1
        for i in range(x.shape[1]):
            for j in range(x.shape[2]):
                x_new, y_new, static_new = batcher_lstm(x[:, i, j, :], y[:, i, j, :], static[i, j, :],
                                                        cfg["seq_len"], cfg['forcast_time'])
                # if np.sum(x_new)!=0:
                # print('x_new are',x_new)
                x_new = torch.from_numpy(x_new).to(device)
                static_new = torch.from_numpy(static_new).to(device)
        ...
        # ------------------------------------------------------------------------------------------------------------------------------
# for each grid by lstm model
    if cfg["modelname"] in ['AttentionLSTM']:
        count = 1
        for i in range(x.shape[1]):
            for j in range(x.shape[2]):
                x_new, y_new, static_new = batcher_lstm(x[:, i, j, :], y[:, i, j, :], static[i, j, :],
...
# ------------------------------------------------------------------------------------------------------------------------------
# for each grid by lstm model
    if cfg["modelname"] in ['LSTMAttentionEncoderDecoder']:
        count = 1
        for i in range(x.shape[1]):
            for j in range(x.shape[2]):
                x_new, y_new, static_new = batcher_lstm(x[:, i, j, :], y[:, i, j, :], static[i, j, :],
