...
# ------------------------------------------------------------------------------------------------------------------------------
	#Partition and create datasets
        ...
        if not os.path.exists(PATH + 'x_train.npy'):
            ...
            x_train.flush()
            del x_train
        x_train = np.memmap(PATH + 'x_train.npy', dtype=cfg['data_type'], mode='r+', shape=(
            self.time_length_f - N, self.nlat_f, self.nlon_f, self.num_features_f + self.num_features_l))
        ...
        if not os.path.exists(PATH + 'x_test.npy'):
            x_test = np.memmap(PATH + 'x_test.npy', dtype=cfg['data_type'], mode='w+',
                               shape=(N, self.nlat_f, self.nlon_f, self.num_features_f + self.num_features_l))
            x_test[:, :, :, :self.num_features_f], x_test[:, :, :, self.num_features_f:] = forcing[
                                                                                           self.time_length_f - N:], land_surface[
                                                                                                                     self.time_length_f - N:]
            x_test.flush()
            del x_test
        x_test = np.memmap(PATH + 'x_test.npy', dtype=cfg['data_type'], mode='r+',
                           shape=(N, self.nlat_f, self.nlon_f, self.num_features_f + self.num_features_l))
